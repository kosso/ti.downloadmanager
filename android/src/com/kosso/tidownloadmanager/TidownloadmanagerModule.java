...
public class TidownloadmanagerModule extends KrollModule
{
							...
							if (cursor.moveToFirst()) {
								...
								if (cursor.getInt(cursor.getColumnIndex(DownloadManager.COLUMN_STATUS)) == DownloadManager.STATUS_SUCCESSFUL) {
									...
									if(hashMapOfRequestedThreads.size() == 0){
										...
										// Log.d(LCAT, "All Downloads look complete: shutdown the executorService");
									...
									hashMapOfRequestedThreads.remove(dId); // delete from HashMap
									// Log.d(LCAT, "hashMapOfRequestedThreads count: " + TiConvert.toString(hashMapOfRequestedThreads.size()));
									if(hashMapOfRequestedThreads.size() == 0){
										// shutdown the executorService
										// Log.d(LCAT, "All Downloads look done (with failures): shutdown the executorService");
										executorService.shutdown();
										executorService = null;
									}
									break;
	...
	public void error(String downloadId, String sourceUrl, String filePath, String fileName) {
		// Log.d(LCAT,"Download error");
	...
	public void done(String downloadId, String sourceUrl, int bytesTotal, String filePath, String fileName) {
		HashMap<String,String> event = new HashMap<String, String>();
		event.put("uid",downloadId);
		event.put("path",filePath);
		event.put("filename",fileName);
	...
	public void downloadFile(KrollDict dict) {
		...
		// Since we make the downloads invisible to the standard UI, we don't need these. iOS does not support them.
	...
	public void cancelDownload(String uid) {
		...
		if(f != null){
			...
			// Log.d(LCAT, "in cancel single: cancelling runnable: " + cancelled);
		}
		f = null;
		hashMapOfRequestedThreads.remove(Long.parseLong(uid));  // delete from HashMap

		// Log.d(LCAT, "in cancel single: hashMapOfRequestedThreads: " + TiConvert.toString(hashMapOfRequestedThreads.size()));
		if(hashMapOfRequestedThreads.size() == 0){
			// shutdown the executorService
			// Log.d(LCAT, "in cancel single: KILL THE EXECUTOR!");
	...
	public void cancelAllDownloads() {
		...
		while (c.moveToNext()) {
			if(TiConvert.toInt(c.getString(c.getColumnIndex(DownloadManager.COLUMN_BYTES_DOWNLOADED_SO_FAR))) < TiConvert.toInt(c.getString(c.getColumnIndex(DownloadManager.COLUMN_TOTAL_SIZE_BYTES)))){
				// Incomplete.. 
				// Log.d(LCAT, "in cancel all: removing incompleted download: uid: "+TiConvert.toString(c.getLong(c.getColumnIndex(DownloadManager.COLUMN_ID))));
				...
				// Log.d(LCAT, "removed from downloads: " + removed);
				// Log.d(LCAT, "in cancel all: hashMapOfRequestedThreads: " + TiConvert.toString(hashMapOfRequestedThreads.size()));		
			}
		}
		c.close();
