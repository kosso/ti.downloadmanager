...
#define TAG "com.kosso.tidownloadmanager"

using namespace v8;

static Persistent<Object> bindingCache;

static void Tidownloadmanager_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Tidownloadmanager.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::TidownloadmanagerBindings::lookupGeneratedInit(
...
static void Tidownloadmanager_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Tidownloadmanager_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Tidownloadmanager_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::TidownloadmanagerBindings::lookupGeneratedInit(*binding, bindingLength);
...
Java_com_kosso_tidownloadmanager_TidownloadmanagerBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.kosso.tidownloadmanager", &TidownloadmanagerBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::TidownloadmanagerBindings::lookupGeneratedInit));
}
